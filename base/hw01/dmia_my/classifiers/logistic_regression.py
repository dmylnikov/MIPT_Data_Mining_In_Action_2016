...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        y_proba = np.vstack((1-self.h(X), self.h(X))).T
    ...
    def h(self, X):
        assert X.shape[1] == self.w.shape[0], 'assert %i %i'%(X.shape[1], self.w.shape[0])
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        # entropy loss
        loss = -np.sum(y_batch * np.log(self.h(X_batch)) + (1 - y_batch) * np.log(1 - self.h(X_batch)))
        dw = X_batch.T.dot(self.h(X_batch) - y_batch)
        # logloss
        #H = 1 + np.exp(-X_batch.dot(self.w) * y_batch)
        #loss = np.sum(np.log(H))
